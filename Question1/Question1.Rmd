...
```{r, warning =  FALSE, fig.align = 'center', fig.cap = "Persistence of National Top 25 Baby Names (1910-2014) by Spearman rank-correlation.\\label{Figure1}", fig.ext = 'png', fig.height = 5, fig.width = 10}

PersistPlot <- plot_name_persistence(corr_results = corr_results) 
PersistPlot

```
Figure \ref{Figure1} indicates that name persistence is much stronger among names for boys than names for girls. Both display trends that appear to be generally downward sloping over time. Persistence is also seen to decrease the further the future year is from the target year. Considering the client's suspicion (that popular name trends have been slower to persist since the 1990s than in earlier decades) is correct for boys names, but appears to be the opposite for girls names which display with an unexpected upwards trend since the 1990s. 
