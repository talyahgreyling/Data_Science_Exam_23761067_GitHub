...
pacman::p_load(dplyr, ggplot2, tidyverse, stringr, purrr) ############## update 
...
df_combined_unique <- df_combined %>% 
    filter(!contains_duplicate_word(df_combined$name),
           !contains_duplicate_word(album),
           !contains_duplicate_word(album_name))
           
# Test each column's filter
name_issues <- df_combined %>% filter(contains_duplicate_word(df_combined$name))
album_issues <- df_combined %>% filter(contains_duplicate_word(album))
album_name_issues <- df_combined %>% filter(contains_duplicate_word(album_name))

# Inspect the results
nrow(name_issues)      # If == nrow(df_combined), ALL names are flagged
nrow(album_issues)     # Same for albums
nrow(album_name_issues)     
