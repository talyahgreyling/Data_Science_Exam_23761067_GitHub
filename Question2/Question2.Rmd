...
Author1: "Talyah Greyling^[__Contributions:__  \\newline _The authors would like to thank the cleaning staff in Van der Sterr for turning on the heating in the normally arctic lab._]"  # First Author - note the thanks message displayed as an italic footnote of first page.
...
# Source in all existing code folders 
list.files('code/', full.names = T, recursive = T) %>% as.list() %>% walk(~source(.))
list.files('..//Question1/code/', full.names = T, recursive = T) %>% as.list() %>% walk(~source(.))

# Load data
Loc <- "data/Coldplay_vs_Metallica/" # set up directory path to data 
coldplay <- read_csv(glue::glue("{Loc}Coldplay.csv")) # read in csv file
metallica <- read_csv(glue::glue("{Loc}metallica.csv")) 
spotify <- read_rds(glue::glue("{Loc}Broader Spotify Info.rds")) # read in rds file
billboard_100 <- read_rds(glue::glue("{Loc}charts.rds"))
